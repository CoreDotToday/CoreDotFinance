...
from coredotfinance._krx._to_DataFrame import to_DataFrame
from coredotfinance._krx._get_requested_data import get_requested_data


def data_reader(
    code, start=None, end=None, day=None, division=None, symbol=None, **kwargs
):
    requested_data = get_requested_data(code, start, end, day, division, symbol, **kwargs)
...
# refactoring
def data_reader_(
        code, symbol, start, end, **kwargs
):
    data = read_data(code, symbol, start, end, **kwargs)
    columns = get_columns(code, symbol, start, end, **kwargs)
    return to_DataFrame(data, columns)


def read_data(code, symbol, start, end, **kwargs):
    requested_data = get_requested_data(code, start, end, symbol, **kwargs)
    mdcstat = parse_mdcstat(requested_data)
    jsp_soup = get_jsp_soup(mdcstat)
    valid_requested_data = convert_valid_requested_data(jsp_soup, requested_data)
    krx_data = get_krx_data(valid_requested_data)
...
def get_columns(code, symbol, start, end, **kwargs):
    pass
