...
class Oil(Commodity):  # 석유 [16101~16105]
    def __init__(self, code, start, end, day, product, **kwargs):
        ...
        self.secugrpld = secugrpld_map[kwargs.get('secugrpld', None)]
        self.secugrpld_1 = kwargs.get('secugrpld', None)  # secugrpld: GA(휘발유), DI(경유), KE(등유)

    def info_of_entire_items(self):  # [16101] 전종목 기본정보
        data = {
            'bld': 'dbms/ MDC/STAT/standard/MDCSTAT14301',
            'secugrpld': self.secugrpld_1,  # ALL(전체), GA(휘발유), DI(경유), KE(등유)
        }
    ...
    def trade_performance_per_investor(self):  # [16103] 참가자별 거래실적
        data = {
            ...
            'secugrpId': self.secugrpld_1,  # GA(휘발유), DI(경유), KE(등유)
            'strtDd': self.start,
            'endDd': self.end,
            #  'share': 1,  # 1~3(L / 천L / 백만L)
            #  'money': 1,  # 1~4(원 / 천원 / 백만원 / 십억원)
...
class Gold(Commodity):  # 금 [16201~16207]
    def __init__(self, code, start, end, day, product, **kwargs):
        ...
        self.secugrpld_1 = self.kwargs.get('secugrpld', None)  # secugrpld: GA(휘발유), DI(경유), KE(등유)
