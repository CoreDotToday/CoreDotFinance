...
def get_requested_data(code, start, end, day, division, item, **kwargs):
    ...
    if code in index_code_list_stock:
        requested_data = StockIndex(code, start, end, day, division, item, **kwargs).get_requested_data()
        ...
        requested_data = DerivationIndex(code, start, end, day, division, item).get_requested_data()

    elif code in stock_code_list_item:
        requested_data = stock.ItemPrice(code, start, end, day, division, item, **kwargs).get_requested_data()
    elif code in stock_code_list_info:
        requested_data = stock.ItemInfo(code, start, end, day, division).get_requested_data()
    elif code in stock_code_list_trade:
        requested_data = stock.TradePerform(code, start, end, day, division, item, **kwargs).get_requested_data()
    elif code in stock_code_list_others:
        requested_data = OtherSecurity(code, start, end, day, division).get_requested_data()
    elif code in stock_code_list_detail:
        requested_data = Detail(code, start, end, day, division, item, **kwargs).get_requested_data()
        ...
        requested_data = bond.ItemPrice(code, start, end, day, item, **kwargs).get_requested_data()
    elif code in bond_code_list_info:
        requested_data = bond.ItemInfo(code, start, end, day, item, **kwargs).get_requested_data()
    elif code in bond_code_list_trade:
        requested_data = bond.TradePerform(code, start, end, day, item, **kwargs).get_requested_data()
    elif code in bond_code_list_detail:
        requested_data = bond.Detail(code, start, end, day, item, **kwargs).get_requested_data()

    elif code in derivative_code_list_price:
        requested_data = derivative.ItemPrice(code, start, end, day, item, **kwargs).get_requested_data()
    elif code in derivative_code_list_info:
        requested_data = derivative.ItemInfo(code, start, end, day, item, **kwargs).get_requested_data()
    elif code in derivative_code_list_trade:
        requested_data = derivative.TradePerform(code, start, end, day, item, **kwargs).get_requested_data()
    elif code in derivative_code_list_detail:
        requested_data = derivative.Detail(code, start, end, day, item, **kwargs).get_requested_data()
        ...
        requested_data = CarbonEmission(code, start, end, day, **kwargs).get_requested_data()

    elif code in oversees_code_list_euro:
        requested_data = oversees.EUREX(code, start, end, day, **kwargs).get_requested_data()
